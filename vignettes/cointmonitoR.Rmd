...
At first, you have to install the `cointReg` package, that provides functions for parameter estimation and inference in cointegrating regressions (see its [GitHub page](https://github.com/aschersleben/cointReg) for further information).
Since the two packages are not available on CRAN yet, you have to download and install the packages directly from GitHub.
```{R, eval = FALSE}
devtools::install_github("aschersleben/cointReg", build_vignettes = TRUE)
```

Then install the `cointmonitoR` package and load it:
```{R, eval = FALSE}
...
For a detailed view on the test statistics, you can use the base plot argument `log`. It reveals that the test statistic is starting to grow immediately after observation no. 61, which is the first one after the structural break.
